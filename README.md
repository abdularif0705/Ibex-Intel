...
Direct Scraping & Signal Scoring (Python + TLS Fingerprinting + NLP):
...
└─ NLP signal stack: scikit-learn TF-IDF + optional PyTorch MiniLM embeddings
...
│                                            │
│ ┌─ Direct Scraping + NLP (Python svc)      │
│ │  └─ LinkedIn, Indeed, Glassdoor          │
│ │     (curl_cffi TLS spoofing + scikit-    │
│ │      learn TF-IDF + optional PyTorch     │
│ │      MiniLM embeddings)                  │
│ │                                          │
│ ├─ API Integration                         │
...
**Python NLP Signal Stack:**

- The same Flask microservice (`python-scraper/main.py`) pipes sanitized text into `python-scraper/similarity.py`, where a scikit-learn TF-IDF + cosine pipeline (<512 MB footprint) scores 824 domain-specific signals and feeds the stage classifier.
- When deeper semantics are needed, `python-scraper/similarity_vector.py` lights up a PyTorch-powered `sentence-transformers/all-MiniLM-L6-v2` encoder so we can run embedding search out of the same container; the optional `/api/v1/detect-signals-vector` endpoint is already stubbed.
- Rollout plan: keep TF-IDF as the default low-cost path, then toggle the PyTorch stack (install dependency + enable endpoint flag) for premium scans or GPU nodes without touching the frontend contract.
...
| **Scraping & NLP Service** | Python (Flask + curl_cffi + scikit-learn + optional PyTorch MiniLM) in Docker on Render |
